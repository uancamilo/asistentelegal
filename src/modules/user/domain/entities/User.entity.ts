...
export class UserEntity {
   ...
   * Basado en la matriz de permisos confirmada
   */
  canEditAccount(_accountId: string, accountName: string, accountOwnerId: string): boolean {
    // SUPER_ADMIN puede editar todas las cuentas (incluida "Employees")
    if (this.role === Role.SUPER_ADMIN) {
      return true;
    }

    // ADMIN puede editar cuentas de clientes (NO "Employees")
    if (this.role === Role.ADMIN) {
      if (accountName === 'Employees') {
        return false; // ADMIN no puede editar cuenta "Employees"
      }
      return true; // Puede editar otras cuentas de clientes
    }
